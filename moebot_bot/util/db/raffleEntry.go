...
const (
	raffleQuery = `SELECT id, GuildUid, UserUid, RaffleType, TicketCount, RaffleData
	...
	raffleUpdate = `UPDATE raffle_entry SET RaffleData = $1, TicketCount = TicketCount + $2 WHERE id = $3`
...
func RaffleEntryUpdate(entry RaffleEntry, ticketAdd int) error {
	_, err := moeDb.Exec(raffleUpdate, entry.RaffleData, ticketAdd, entry.id)
	if err != nil {
...
func RaffleEntryQuery(userUid string, guildUid string) (raffleEntries []RaffleEntry, err error) {
	...
	for rows.Next() {
		var re RaffleEntry
		if err := rows.Scan(&re.id, &re.GuildUid, &re.UserUid, &re.RaffleType, &re.TicketCount, &re.RaffleData); err != nil {
